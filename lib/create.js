...
const createCSSFontFaceTemplate = (filename, ttfBuffer, config) => {
  ...
  return ttfInfoPromised(ttfBuffer).then((info) => {
    ...
    const cssFontDisplay = config.fontDisplay ? `\n  font-display: ${config.fontDisplay};` : "";

    return `
@font-face {
  font-family: '${styles.fontFamily}';
  font-weight: ${styles.fontWeight};
  font-style: ${styles.fontStyle};${cssFontDisplay}
...
const createFontFace = function createFontFace(filePath, ttfBuffer, config) {
  const dirname = path.dirname(filePath);
  const filename = path.basename(filePath, '.ttf');

  return writeTransformedFonts(dirname, filename, ttfBuffer, config).then(() => {
    return createCSSFontFaceTemplate(filename, ttfBuffer, config);
  });
};
